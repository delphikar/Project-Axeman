...
function MarketplaceImprovement() {
	/**
	 * Initializes object
	 */
	this.Register = function () {
		Log("MarketplaceImprovementMetadata: Registering MarketplaceImprovementMetadata plugin...");
	...
	function FillVillagesList() {
		Log("buildMarketFillVillagesList - Started...");
		...
		Log("buildMarketFillVillagesList - Textbox successfully replaced!");
	}


	function AddTransportShortcuts(traderMaxTransport) {
		Log("buildMarketAddTransportShortcuts - Started...");
		...
		Log("buildMarketAddTransportShortcuts - Finished successfully!");
	}


	function IncomingSum() {
		Log("buildMarketIncomingSum - Generating table...");
		...
		*/
		...
		Log("buildMarketIncomingSum - Finished successfully!");
	}
...
function RegisterTimerFillInJunkResource(args) {
    ...
    Log("buildMarketRegisterTimerFillInJunkResource - Timer set to interval [250]");
}
...
function InsertJunkResourceTable() {
    Log("buildMarketInsertJunkResourceTable - Inserting Junk resources table...");
    ...
    Log("buildMarketInsertJunkResourceTable - Junk resources table inserted successfully...");
}
...
function FillInJunkResourceTimer(args) {
    ...
    Log("buildMarketFillInJunkResourceTimer - traders [" + args[0] + "] each [" + args[1] + "] sending [" + resSum + "] with junk [" + junkAmount + "]");
}

}

// Metadata for this plugin (MarketplaceImprovement)
var MarketplaceImprovementMetadata = {
	Name: "MarketplaceImprovement",
	Alias: "Marketplace Improvement",
	Category: "Utility",
	Version: "0.1",
	Description: "ToDo",
	...
	Class: MarketplaceImprovement
};

// Adds this plugin to global list of available plugins
GlobalPluginsList[GlobalPluginsList.length] = $.extend(true, {}, Models.PluginMetadata, MarketplaceImprovementMetadata);
